...
// libavcodec/apedec.c from FFmpeg (commit c966a1d), outside libavcodec. The
// entropy decoders, predictors and filters are FFmpeg's; where this file
// departs from them, a comment at that spot says how.
...
// The bit reader's widest single read.
...
// A big-endian bit reader over a whole frame, for files before 3.90. Reads past
// the end return zeros, like libavcodec's padded reader.
...
typedef struct APEContext {
	...
	const uint8_t *data_end; // frame data end
	const uint8_t *ptr;		 // current position in frame data
	ape_refill_fn refill;
	void *refill_arg;
...
// At least `n` bytes ahead of ptr, asking the refill callback while short.
static int ensure_bytes(APEContext *ctx, int n) {
	while (ctx->data_end - ctx->ptr < n) {
		if (!ctx->refill || !ctx->refill((ape_core_t *)ctx, ctx->refill_arg)) {
			return 0;
		}
	}
	return 1;
...
static inline int next_byte(APEContext *ctx) {
	if (ctx->ptr >= ctx->data_end && !ensure_bytes(ctx, 1)) {
		ctx->error = 1;
		return 0;
	}
	return *ctx->ptr++;
...
static inline uint32_t next_be32(APEContext *ctx) {
	uint32_t v = (uint32_t)ctx->ptr[0] << 24 | (uint32_t)ctx->ptr[1] << 16 | (uint32_t)ctx->ptr[2] << 8 | ctx->ptr[3];
	ctx->ptr += 4;
	return v;
}

static void init_get_bits8(GetBitContext *gb, const uint8_t *buf, int bytes) {
	if (bytes < 0 || bytes > INT_MAX / 8) {
		bytes = 0;
	}
	gb->buffer = buf;
	gb->size_in_bits = bytes * 8;
	gb->index = 0;
}
...
static inline unsigned get_bits1(GetBitContext *gb) {
	int i = gb->index;
	if (i >= gb->size_in_bits) {
		return 0;
	}
	gb->index = i + 1;
	return (gb->buffer[i >> 3] >> (7 - (i & 7))) & 1;
}
...
static inline unsigned get_bits_long(GetBitContext *gb, int n) { return get_bits(gb, n); }

static inline void skip_bits_long(GetBitContext *gb, int n) {
	int64_t i = (int64_t)gb->index + n;
	gb->index = i > gb->size_in_bits ? gb->size_in_bits : (i < 0 ? 0 : (int)i);
}

static inline int get_unary(GetBitContext *gb, int stop, int len) {
	int i;

	for (i = 0; i < len && (int)get_bits1(gb) != stop; i++)
		;
	return i;
}

static inline int ape_log2(unsigned v) {
	int n = 0;
	while (v >>= 1) {
		n++;
	}
...
void ape_core_free(ape_core_t *core) {
	...
	free(s->decoded_buffer);
	free(s->interim_buffer);
	free(core);
}

ape_core_t *ape_core_new(int fileversion, int compression, int flags, int bps, int channels) {
	if (fileversion < 3800 || fileversion > 3990 || channels < 1 || channels > MAX_CHANNELS) {
		return NULL;
	}
	if (compression % 1000 || compression > COMPRESSION_LEVEL_INSANE || !compression ||
		(fileversion < 3930 && compression == COMPRESSION_LEVEL_INSANE)) {
		return NULL;
	}

	ape_core_t *core = calloc(1, sizeof(*core));
	if (!core) {
		return NULL;
	}
	APEContext *s = &core->s;
	...
	s->compression_level = compression;
	s->flags = flags;
	...
	for (int i = 0; i < APE_FILTER_LEVELS; i++) {
		if (!ape_filter_orders[s->fset][i])
			break;
		if (!(s->filterbuf[i] = malloc((ape_filter_orders[s->fset][i] * 3 + HISTORY_SIZE) * 4))) {
			ape_core_free(core);
			return NULL;
		}
	}
...
bool ape_core_whole_frames(const ape_core_t *core) { return core->s.fileversion < 3930; }
...
// Cumulative frequency for the next symbol, without updating: tot_f is the
// total frequency or (code_value)1 << shift.
...
// Update decoding state: sy_f the interval length (frequency of the symbol),
...
static const uint16_t counts_3970[22] = {
	...
	64878, 65126, 65276, 65365, 65419, 65450, 65469, 65480, 65487, 65491, 65493,
};
...
static const uint16_t counts_3980[22] = {
	...
	65416, 65447, 65466, 65476, 65482, 65485, 65488, 65490, 65491, 65492, 65493,
};
...
// Decode symbol: counts is the probability range start position, counts_diff
// the probability range widths.
static inline int range_get_symbol(APEContext *ctx, const uint16_t counts[], const uint16_t counts_diff[]) {
	int symbol, cf;
...
static inline int ape_decode_value_3860(APEContext *ctx, GetBitContext *gb, APERice *rice) {
...
static inline int ape_decode_value_3900(APEContext *ctx, APERice *rice) {
	...
	if (tmpk <= 16 || ctx->fileversion < 3910) {
		if (tmpk > 23) {
			ctx->error = 1; // FFmpeg passes the value on; here it ends the frame
			return -1;
		}
...
static int get_k(int ksum) { return ape_log2(ksum) + !!ksum; }
...
static int init_entropy_decoder(APEContext *ctx) {
	...
	if (ctx->fileversion >= 3900) {
		if (!ensure_bytes(ctx, 6))
			return -1;
		ctx->CRC = next_be32(ctx);
	} else {
	...
	if ((ctx->fileversion > 3820) && (ctx->CRC & 0x80000000)) {
		...
		// Before 3.90 the flags follow the CRC in the bitstream, which is
		// where Monkey's Audio reads them; FFmpeg reads the byte pointer,
		// still at the start of the frame for these versions.
		if (ctx->fileversion >= 3900) {
			if (!ensure_bytes(ctx, 6))
				return -1;
			ctx->frameflags = (int)next_be32(ctx);
		} else {
			ctx->frameflags = (int)get_bits_long(&ctx->gb, 32);
		}
	}
...
static inline int filter_fast_3320(APEPredictor *p, const int decoded, const int filter, const int delayA) {
	int32_t predictionA;
...
static inline int filter_3800(APEPredictor *p, const unsigned decoded, const int filter, const int delayA,
							  const int delayB, const int start, const int shift) {
	int32_t predictionA, predictionB, sign;
	int32_t d0, d1, d2, d3, d4;

	p->buf[delayA] = p->lastA[filter];
	p->buf[delayB] = p->filterB[filter];
	if (p->sample_pos < (unsigned)start) {
	...
	predictionA = d0 * p->coeffsA[filter][0] + d1 * p->coeffsA[filter][1] + d2 * p->coeffsA[filter][2];
	...
	predictionB = d3 * p->coeffsB[filter][0] - d4 * p->coeffsB[filter][1];
...
static void predictor_decode_stereo_3800(APEContext *ctx, int count) {
	...
	while (count--) {
		...
		if (ctx->compression_level == COMPRESSION_LEVEL_FAST) {
			...
			*decoded0 = filter_3800(p, Y, 0, YDELAYA, YDELAYB, start, shift);
			decoded0++;
			*decoded1 = filter_3800(p, X, 1, XDELAYA, XDELAYB, start, shift);
			decoded1++;
		}
...
static void predictor_decode_mono_3800(APEContext *ctx, int count) {
	...
	while (count--) {
		if (ctx->compression_level == COMPRESSION_LEVEL_FAST) {
			...
			*decoded0 = filter_3800(p, *decoded0, 0, YDELAYA, YDELAYB, start, shift);
			decoded0++;
		}
...
static inline int predictor_update_3930(APEPredictor *p, const int decoded, const int filter, const int delayA) {
...
static inline int predictor_update_filter(APEPredictor64 *p, const int decoded, const int filter, const int delayA,
										  const int delayB, const int adaptA, const int adaptB, int interim_mode) {
	...
	predictionA = p->buf[delayA] * p->coeffsA[filter][0] + p->buf[delayA - 1] * p->coeffsA[filter][1] +
				  p->buf[delayA - 2] * p->coeffsA[filter][2] + p->buf[delayA - 3] * p->coeffsA[filter][3];
	...
	predictionB = p->buf[delayB] * p->coeffsB[filter][0] + p->buf[delayB - 1] * p->coeffsB[filter][1] +
				  p->buf[delayB - 2] * p->coeffsB[filter][2] + p->buf[delayB - 3] * p->coeffsB[filter][3] +
	...
	return (int)p->filterA[filter];
}

// p->buf is put back at the start of the history on every call. That holds only
// because every call but a frame's last covers a multiple of HISTORY_SIZE
// blocks (APE_CORE_CHUNK), and so ends with the history just moved there.
static void predictor_decode_stereo_3950(APEContext *ctx, int count) {
	...
	for (int pass = 0; pass < num_passes; pass++) {
		...
		while (count--) {
			// Predictor Y
			int32_t a0 =
				predictor_update_filter(p, *decoded0, 0, YDELAYA, YDELAYB, YADAPTCOEFFSA, YADAPTCOEFFSB, interim_mode);
			int32_t a1 =
				predictor_update_filter(p, *decoded1, 1, XDELAYA, XDELAYB, XADAPTCOEFFSA, XADAPTCOEFFSB, interim_mode);
			*decoded0++ = a0;
			*decoded1++ = a1;
			if (num_passes > 1) {
...
static void predictor_decode_mono_3950(APEContext *ctx, int count) {
	...
	while (count--) {
		...
		predictionA = (int32_t)(p->buf[YDELAYA] * p->coeffsA[0][0] + p->buf[YDELAYA - 1] * p->coeffsA[0][1] +
								p->buf[YDELAYA - 2] * p->coeffsA[0][2] + p->buf[YDELAYA - 3] * p->coeffsA[0][3]);

		currentA = (int32_t)(A + (uint64_t)(predictionA >> 10));
...
// The dot product of v1 and v2, then v1 += mul * v3, with mul -1, 0 or 1. The
// sum wraps at 32 bits; it is kept in 64 so that MIPS32 accumulates it with
// madd, and only its low 32 bits are returned. v1 takes the sum in int16, which
// wraps the same way the store does.
static int32_t scalarproduct_and_madd_int16(int16_t *v1, const int16_t *v2, const int16_t *v3, int order, int mul) {
	...
	if (mul > 0) {
		for (int i = 0; i < order; i += 4) {
			int c0 = v1[i], c1 = v1[i + 1], c2 = v1[i + 2], c3 = v1[i + 3];
			res += (int64_t)c0 * v2[i];
			res += (int64_t)c1 * v2[i + 1];
			res += (int64_t)c2 * v2[i + 2];
			res += (int64_t)c3 * v2[i + 3];
			v1[i] = (int16_t)(c0 + v3[i]);
			v1[i + 1] = (int16_t)(c1 + v3[i + 1]);
			v1[i + 2] = (int16_t)(c2 + v3[i + 2]);
			v1[i + 3] = (int16_t)(c3 + v3[i + 3]);
		}
	} else if (mul < 0) {
		for (int i = 0; i < order; i += 4) {
			int c0 = v1[i], c1 = v1[i + 1], c2 = v1[i + 2], c3 = v1[i + 3];
		...
		for (int i = 0; i < order; i += 4) {
			res += (int64_t)v1[i] * v2[i];
			res += (int64_t)v1[i + 1] * v2[i + 1];
			res += (int64_t)v1[i + 2] * v2[i + 2];
			res += (int64_t)v1[i + 3] * v2[i + 3];
		}
	}
...
static void do_apply_filter(int version, APEFilter *f, int32_t *data, int count, int order, int fracbits) {
	...
	while (count--) {
		// Round the fixed-point scalar product. The half is added in 32 bits,
		// wrapping, as Monkey's Audio 3.99 does: FFmpeg adds it in 64, and the
		// two differ when the product is within half a step of the 32-bit
		// limit, which loud 24-bit audio at the insane level reaches.
		res = scalarproduct_and_madd_int16(f->coeffs, f->delay - order, f->adaptcoeffs - order, order,
										   APESIGN(*data));
		res = (int)((unsigned)res + (1u << (fracbits - 1))) >> fracbits;
		res += (unsigned)*data;
		*data++ = res;
		...
		*f->delay++ = clip_int16(res);
		...
		if (version < 3980) {
			...
			absres = FFABSU(res);
			if (absres)
				*f->adaptcoeffs = APESIGN(res) * (8 << ((absres > f->avg * 3LL) + (absres > (f->avg + f->avg / 3))));
			else
				*f->adaptcoeffs = 0;
		...
		if (f->delay == f->historybuffer + HISTORY_SIZE + (order * 2)) {
			memmove(f->historybuffer, f->delay - (order * 2), (order * 2) * sizeof(*f->historybuffer));
			f->delay = f->historybuffer + order * 2;
			f->adaptcoeffs = f->historybuffer + order;
		}
	}
...
static void apply_filter(APEContext *ctx, APEFilter *f, int32_t *data0, int32_t *data1, int count, int order,
						 int fracbits) {
...
static void ape_apply_filters(APEContext *ctx, int32_t *decoded0, int32_t *decoded1, int count) {
	...
	for (i = 0; i < APE_FILTER_LEVELS; i++) {
		if (!ape_filter_orders[ctx->fset][i])
			break;
		apply_filter(ctx, ctx->filters[i], decoded0, decoded1, count, ape_filter_orders[ctx->fset][i],
...
int ape_core_start_frame(ape_core_t *core, const uint8_t *data, const uint8_t *end, uint32_t skip, uint32_t nblocks,
						 ape_refill_fn refill, void *arg) {
	APEContext *s = &core->s;
	...
	if (s->fileversion >= 3900) {
		...
		if (!ensure_bytes(s, (int)skip)) {
			return -1;
		}
		s->ptr += skip;
	} else {
		init_get_bits8(&s->gb, s->ptr, (int)(s->data_end - s->ptr));
		if (s->fileversion > 3800)
			skip_bits_long(&s->gb, (int)skip * 8);
		else
			skip_bits_long(&s->gb, (int)skip);
	}
	...
	s->samples = (int)nblocks;
	return 0;
}

// Room for `count` blocks per channel in the output buffers, and in the interim
// ones while the interim mode of a 24-bit stream is still undecided.
static int ensure_buffers(APEContext *s, int count) {
	int need = 2 * FFALIGN(count, 8);
	if (need > s->decoded_size) {
		int32_t *grown = realloc(s->decoded_buffer, (size_t)need * sizeof(*grown));
		if (!grown) {
			return -1;
		}
		s->decoded_buffer = grown;
		s->decoded_size = need;
	}
	memset(s->decoded_buffer, 0, (size_t)need * sizeof(*s->decoded_buffer));
	s->decoded[0] = s->decoded_buffer;
	s->decoded[1] = s->decoded_buffer + FFALIGN(count, 8);

	if (s->interim_mode < 0) {
		if (need > s->interim_size) {
			int32_t *grown = realloc(s->interim_buffer, (size_t)need * sizeof(*grown));
			if (!grown) {
				return -1;
			}
			s->interim_buffer = grown;
			s->interim_size = need;
		}
		memset(s->interim_buffer, 0, (size_t)need * sizeof(*s->interim_buffer));
		s->interim[0] = s->interim_buffer;
		s->interim[1] = s->interim_buffer + FFALIGN(count, 8);
	}
	return 0;
}
...
int ape_core_decode(ape_core_t *core, int count, const int32_t **left, const int32_t **right) {
	...
	if (count <= 0 || count > s->samples) {
		return -1;
	}
	if (ensure_buffers(s, count) < 0) {
		return -1;
	}
	...
	if ((s->channels == 1) || (s->frameflags & APE_FRAMECODE_PSEUDO_STEREO))
		ape_unpack_mono(s, count);
	else
		ape_unpack_stereo(s, count);
	...
	s->samples -= count;
	*left = s->decoded[0];
	*right = s->channels == 2 ? s->decoded[1] : s->decoded[0];
	return 0;
}
